...
def check_biometric_data(h, w, c):
    if abs(OK_HEIGHT - h) > 5 or abs(OK_WEIGHT - w) > 5 or c is not OK_CARD:
        return False
    return True
...
def start_mqtt_client(broker_addr):
    ...
    while True:
        random_height = random.randint(170, 190)
        random_weight = random.randint(70, 90)
        random_num_for_card = random.randint(0,1)
        random_card = True if random_num_for_card is 1 else False

        check = check_biometric_data(random_height, random_weight, random_card)
        data_content = "success" if check else "error"

        client.publish("topic/door", '{success:' + data_content + '}')
        time.sleep(10)
